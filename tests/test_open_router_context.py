"""
tests/test_open_router_context.py

Unit tests for open_router_context.py covering:
  - _build_batch_prompt()  — pure string-building function
  - enrich_gbif_results_with_openrouter_batch() — OpenRouter call, JSON parsing, fallback
...
from open_router_context import (
    DEFAULT_MODEL,
    _build_batch_prompt,
    enrich_gbif_results_with_openrouter_batch,
)
...
_VALID_OPENROUTER_JSON = json.dumps({
    "species_context": [
        {
            "scientific_name": "Myotis sodalis",
...
def _make_client(output_text: str) -> MagicMock:
    """Build a minimal mock OpenRouter client whose responses.create() returns output_text."""
    ...
    mock_response.choices[0].message.content = output_text
...
# ---------------------------------------------------------------------------
# 2. enrich_gbif_results_with_openrouter_batch()
# ---------------------------------------------------------------------------
...
class TestEnrichGbifResults:
    ...
    def test_empty_hits_skips_api_call(self):
        mock_client = _make_client(_VALID_OPENROUTER_JSON)
        enrich_gbif_results_with_openrouter_batch(_NO_SPECIES, client=mock_client)
        mock_client.chat.completions.create.assert_not_called()

    def test_empty_hits_returns_empty_species_context(self):
        result = enrich_gbif_results_with_openrouter_batch(
            _NO_SPECIES, client=_make_client(_VALID_OPENROUTER_JSON)
        )
    ...
    def test_valid_response_parsed_correctly(self):
        result = enrich_gbif_results_with_openrouter_batch(
            _ONE_SPECIES, client=_make_client(_VALID_OPENROUTER_JSON)
        )
    ...
    def test_api_called_exactly_once(self):
        mock_client = _make_client(_VALID_OPENROUTER_JSON)
        enrich_gbif_results_with_openrouter_batch(_ONE_SPECIES, client=mock_client)
    ...
    def test_custom_model_passed_to_api(self):
        mock_client = _make_client(_VALID_OPENROUTER_JSON)
        enrich_gbif_results_with_openrouter_batch(
            _ONE_SPECIES, model="nvidia/nemotron-3.5-lightning:free", client=mock_client
        )
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs["model"] == "nvidia/nemotron-3.5-lightning:free"
...
# ---------------------------------------------------------------------------
# 3. enrich_gbif_results_with_openrouter_batch() — OpenRouter API error handling
# ---------------------------------------------------------------------------

def _make_raising_client(exception: Exception) -> MagicMock:
    """Build a mock OpenRouter client whose responses.create() raises the given exception."""
    mock_client = MagicMock()
...
class TestEnrichGbifResultsApiErrors:
    """
    Verify that every OpenRouter API failure mode returns a gracefully-degraded
    ...
    def test_auth_error_does_not_raise(self):
        ...
        result = enrich_gbif_results_with_openrouter_batch(_ONE_SPECIES, client=client)
        assert "species_context" in result

    def test_auth_error_returns_one_entry_per_hit(self):
        from openai import AuthenticationError
        client = _make_raising_client(AuthenticationError("invalid key", response=MagicMock(), body={}))
        result = enrich_gbif_results_with_openrouter_batch(_TWO_SPECIES, client=client)
    ...
    # -- APIStatusError: unexpected HTTP status from OpenRouter --
