"""
open_router_context.py

Generate short species-specific construction context for screening using the OpenRouter API.

Called from GBIF.py after the endangered-species search is done. Takes the list of
flagged species hits, sends them all to OpenRouter in one prompt, and gets back structured
...
even if OpenRouter is unreachable.
...
def enrich_gbif_results_with_openrouter_batch(
    ...
    *,
    ...
    # UI can still show species names and the disclaimer even when OpenRouter is down
    def _error_result(error_tag: str, message: str) -> Dict[str, Any]:
        logger.error("OpenRouter enrichment failed [%s]: %s", error_tag, message)
        return {
            "input": gbif_result.get("input", {}),
            "species_context": [
                {
    ...
    try:
        ...
        return _error_result(
            "auth",
            "OpenRouter API key is invalid or missing. AI ecological context is unavailable.",
        )
    except RateLimitError:
        return _error_result(
            "rate_limit",
            "OpenRouter usage quota exceeded. AI ecological context is temporarily unavailable.",
        )
    except APITimeoutError:
        return _error_result(
            "timeout",
            "OpenRouter request timed out. AI ecological context is unavailable for this scan.",
        )
    except APIConnectionError:
        return _error_result(
            "connection",
            "Could not reach the OpenRouter API. Check network connectivity. AI ecological context is unavailable.",
        )
    ...
    try:
        ...
        logger.warning("OpenRouter returned non-JSON output; storing raw text as fallback")
        parsed = {
            "species_context": [
                {
